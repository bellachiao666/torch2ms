...
def test_pytorch_yolo_fragment():
    ...
    def old_way(tensor, num_classes, num_anchors, anchors, stride_h, stride_w):
        ...
        # Results
        _scale = torch.Tensor([stride_w, stride_h] * 2).type(FloatTensor)  # 'torch.Tensor' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;; 'torch.Tensor.type' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        output = mint.cat(
