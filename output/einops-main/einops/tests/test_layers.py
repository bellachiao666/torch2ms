...
def test_rearrange_imperative():
    for backend in collect_test_backends(symbolic=False, layers=True):
        ...
        for pattern, axes_lengths, input_shape, wrong_shapes in rearrangement_patterns:
            ...
            result_numpy = rearrange(x, pattern, **axes_lengths)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
            layer = backend.layers().Rearrange(pattern, **axes_lengths)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
    ...
    for backend in collect_test_backends(symbolic=False, layers=True):
        ...
        for reduction in REDUCTIONS:
            for pattern, axes_lengths, input_shape, wrong_shapes in reduction_patterns:
                ...
                x /= x.mean()
                result_numpy = reduce(x, pattern, reduction, **axes_lengths)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
                layer = backend.layers().Reduce(pattern, reduction, **axes_lengths)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
                for shape in wrong_shapes:
                    try:
                        layer(backend.from_numpy(np.zeros(shape, dtype="float32")))
                    except BaseException:
...
def create_torch_model(use_reduce=False, add_scripted_layer=False):
    if not is_backend_tested("torch"):
        ...
        return msnn.SequentialCell(
            ...
            Reduce("b c (h h2) (w w2) -> b c h w", "max", h2=2, w2=2) if use_reduce else nn.MaxPool2d(kernel_size = 2),
            nn.Conv2d(6, 16, kernel_size = (5, 5)),
            Reduce("b c (h h2) (w w2) -> b c h w", "max", h2=2, w2=2),
            ms.jit(Rearrange("b c h w -> b (c h w)"))
...
def test_torch_layer():
    if not is_backend_tested("torch"):
        ...
        # tracing (freezing)
        model3 = mint.trace(model2, example_inputs=input)
        ...
        model4 = mint.trace(model2, example_inputs=input)
...
def test_keras_layer():
    ...
    if not is_backend_tested("tensorflow"):
        ...
        def create_keras_model():
            ...
            )  # 'torch.nn.Conv2d':存在 MindSpore 未支持的参数 input_shape，需手动确认;
