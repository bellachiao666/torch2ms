...
class Policy(msnn.Cell):
    ...
    def construct(self, x):
        ...
        return nn.functional.softmax(action_scores, dim = self.dim)
...
class Observer:
    ...
    """
    def __init__(self, batch=True):
        self.id = rpc.get_worker_info().id - 1  # 'torch.distributed.rpc.get_worker_info' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class Agent:
    def __init__(self, world_size, batch=True):
        ...
        for ob_rank in range(1, world_size):
            ...
            self.ob_rrefs.append(remote(ob_info, Observer, args=(batch,)))  # 'torch.distributed.rpc.remote' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
            self.rewards[ob_info.id] = []

        self.states = mint.zeros(len(self.ob_rrefs), 1, 4)
        self.batch = batch
        ...
        self.future_actions = torch.futures.Future()  # 'torch.futures.Future' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
    ...
    def run_episode(self, n_steps=0):
        ...
        rets = torch.futures.wait_all(futs)  # 'torch.futures.wait_all' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        rewards = mint.stack([ret[0] for ret in rets]).cuda().t()
        ep_rewards = sum([ret[1] for ret in rets]) / len(rets)

        if self.batch:
            probs = mint.stack(self.saved_log_probs)
        else:
            probs = [mint.stack(self.saved_log_probs[i]) for i in range(len(rets))]
            probs = mint.stack(probs)
