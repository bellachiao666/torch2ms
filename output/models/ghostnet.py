...
class GhostModuleV3(msnn.Cell):
    def __init__(
        ...
        self.short_conv = msnn.SequentialCell(
            ...
            nn.Conv2d(out_chs, out_chs, kernel_size=(5,1), stride=1, padding=(2,0), groups=out_chs, bias=False, **dd),
            nn.BatchNorm2d(out_chs, **dd),
        ) if self.mode in ['shortcut'] else msnn.Identity()
...
class GhostBottleneck(msnn.Cell):
    ...
    def __init__(
        ...
        self.ghost2 = GhostModule(mid_chs, out_chs, act_layer=msnn.Identity, **dd)
...
class GhostBottleneckV3(msnn.Cell):
    ...
    def __init__(
        ...
        # Squeeze-and-excitation
        self.se = _SE_LAYER(mid_chs, rd_ratio=se_ratio, **dd) if has_se else msnn.Identity()

        # Point-wise linear projection
        self.ghost2 = GhostModuleV3(mid_chs, out_chs, act_layer=msnn.Identity, mode='original', **dd)
...
class GhostNet(msnn.Cell):
    def __init__(
        ...
        stages.append(msnn.SequentialCell(ConvBnAct(prev_chs, out_chs, 1, **dd)))
        self.pool_dim = prev_chs = out_chs

        self.blocks = msnn.SequentialCell(*stages)
        ...
        self.classifier = Linear(out_chs, num_classes, **dd) if num_classes > 0 else msnn.Identity()
