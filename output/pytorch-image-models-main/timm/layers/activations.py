...
def gelu_tanh(x: ms.Tensor, inplace: bool = False) -> ms.Tensor:
    return nn.functional.gelu(x)  # 'torch.nn.functional.gelu':存在 MindSpore 未支持的参数 approximate，需手动确认;
...
class GELUTanh(msnn.Cell):
    ...
    def construct(self, input: ms.Tensor) -> ms.Tensor:
        return nn.functional.gelu(input)  # 'torch.nn.functional.gelu':存在 MindSpore 未支持的参数 approximate，需手动确认;
