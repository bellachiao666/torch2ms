...
class TestAdaptiveAvgMaxPool:
    ...
    def test_adaptive_avgmax_pool2d(self):
        ...
        x = mint.randn(2, 64, 7, 7, device=torch_device)
    ...
    def test_adaptive_avgmax_pool2d_module(self):
        ...
        x = mint.randn(2, 64, 14, 14, device=torch_device)
...
class TestAttentionPool:
    ...
    def test_attention_pool_latent_basic(self):
        ...
        x = mint.randn(2, 49, 64, device=torch_device)
    ...
    def test_attention_pool2d_different_feat_size(self):
        ...
        pool = AttentionPool2d(in_features=64, feat_size=7).to(torch_device)
        for size in [7, 14]:
            x = mint.randn(2, 64, size, size, device=torch_device)
            out = pool(x)
            assert out.shape == (2, 64)
...
class TestPoolingCommon:
    ...
    def test_gradient_flow(self, pool_cls, kwargs, input_shape):
        ...
        x = mint.randn(*input_shape, device=torch_device, requires_grad=True)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
        pool = getattr(layers, pool_cls)(**kwargs).to(torch_device)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
        out = pool(x)
        loss = out.sum()
        loss.backward()
    ...
    def test_torchscript(self, pool_cls, kwargs, input_shape):
        import timm.layers as layers
        x = mint.randn(*input_shape, device=torch_device)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
        pool = getattr(layers, pool_cls)(**kwargs).to(torch_device)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
        pool.eval()
        scripted = ms.jit(pool)
        out_orig = pool(x)
    ...
    def test_different_spatial_sizes(self, pool_cls, kwargs, input_shape):
        ...
        for H, W in [(7, 7), (14, 14), (1, 1), (3, 5)]:
            x = mint.randn(B, C, H, W, device=torch_device)
            out = pool(x)
...
class TestBlurPool:
    ...
    def test_blur_pool_2d_stride(self):
        from timm.layers import BlurPool2d
        x = mint.randn(2, 64, 28, 28, device=torch_device)
