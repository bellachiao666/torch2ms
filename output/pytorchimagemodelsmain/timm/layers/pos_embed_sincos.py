...
# 轻量 torch stub 以兼容原始类型标注/常量
class _TorchStub:
    def __init__(self):
        self.pi = math.pi
        self.dtype = type(ms.float32)
        self.device = str
        self.fx = types.SimpleNamespace(wrap=lambda fn: fn)
...
def pixel_freq_bands(
    ...
    if linear_bands:
        bands = mint.linspace(1.0, max_freq / 2, num_bands, dtype=ms.float32)
    else:
        bands = 2 ** mint.linspace(0, math.log(max_freq, 2) - 1, num_bands, dtype=ms.float32)
...
def freq_bands(
    ...
    exp = mint.arange(0, num_bands, step, dtype=ms.int64).to(ms.float32) / num_bands
