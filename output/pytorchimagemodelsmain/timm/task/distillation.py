...
class LogitDistillationTask(TrainingTask):
    ...
    def prepare_distributed(
        ...
        self.student = DDP(self.student, device_ids=device_ids, **ddp_kwargs)  # 'torch.nn.parallel.DistributedDataParallel' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;; 存在 *args/**kwargs，未转换，需手动确认参数映射;
        return self
...
class FeatureDistillationTask(TrainingTask):
    ...
    def prepare_distributed(
        ...
        self.trainable_module = DDP(self.trainable_module, device_ids=device_ids, **ddp_kwargs)  # 'torch.nn.parallel.DistributedDataParallel' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;; 存在 *args/**kwargs，未转换，需手动确认参数映射;
        return self
