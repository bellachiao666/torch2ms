...
class RepeatAugSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            indices = mint.randperm(len(self.dataset), generator = g)  # 'torch.randperm'默认值不一致(position 3): PyTorch=torch.int64, MindSpore=mindspore.int64;
        else:
        ...
        if isinstance(self.num_repeats, float) and not self.num_repeats.is_integer():
            ...
            indices = indices[ms.Tensor([int(i // self.num_repeats) for i in range(repeat_size)])]  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;
        else:
