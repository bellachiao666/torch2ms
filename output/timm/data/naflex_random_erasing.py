...
class PatchRandomErasing:
        ...
        if self.erase_mode == 'pixel':
            ...
            if self.erase_mode == 'const' or value is not None:
                ...
                if isinstance(erase_value, (int, float)):
                    values = mint.full(size = (shape, erase_value), dtype = dtype)  # 'torch.full':没有对应的mindspore参数 'device' (position 5);
                else:
                    erase_value = ms.Tensor(erase_value, dtype = dtype)  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;; 'torch.tensor':没有对应的mindspore参数 'device' (position 2);
    ...
    def _drop_patches(
        ...
        if self.patch_drop_prob:
            ...
            keep_indices = mint.argsort(mint.randn(size = (1, num_valid)), dim = -1)[:, :num_keep]  # 'torch.randn':没有对应的mindspore参数 'device' (position 5);
    ...
    def _erase_patches(
        ...
        erase_idx = valid_indices[mint.randperm(num_valid)[:num_erase]]  # 'torch.randperm'默认值不一致(position 3): PyTorch=torch.int64, MindSpore=mindspore.int64;; 'torch.randperm':没有对应的mindspore参数 'device' (position 5);
    ...
    def __call__(
        ...
        if patch_valid is None:
            patch_valid = mint.ones((batch_size, num_patches), dtype = ms.bool)  # 'torch.ones':没有对应的mindspore参数 'device' (position 4);
