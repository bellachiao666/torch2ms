...
def get_cache_dir(child_dir: str = ''):
    ...
    hub_dir = get_dir()  # 'torch.hub._get_torch_home.get_dir' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
    child_dir = () if not child_dir else (child_dir,)
    model_dir = os.path.join(hub_dir, 'checkpoints', *child_dir)  # 存在 *args/**kwargs，未转换，需手动确认参数映射;
