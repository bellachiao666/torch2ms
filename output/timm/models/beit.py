...
def gen_relative_position_index(window_size: Tuple[int, int], device=None) -> ms.Tensor:
    ...
    coords = mint.stack(ndgrid(
        mint.arange(window_size[0], dtype = torch.long),
        mint.arange(window_size[1], dtype = torch.long),
    ...
    relative_position_index = mint.zeros(size = (window_area + 1,) * 2, dtype = relative_coords.dtype)  # 'torch.zeros':没有对应的mindspore参数 'device' (position 4);
...
class RelativePositionBias(msnn.Cell):
