...
class MultiScaleBlock(msnn.Cell):
    ...
    def __init__(
        ...
        for d in range(num_branches):
            ...
            if len(tmp) != 0:
                self.blocks.append(msnn.SequentialCell([
                    tmp
                ]))
        ...
        for d in range(num_branches):
            ...
            self.projs.append(msnn.SequentialCell([
                tmp
            ]))
        ...
        for d in range(num_branches):
            ...
            if depth[-1] == 0:  # backward capability:
                ...
                self.fusion.append(msnn.SequentialCell([
                    tmp
                ]))
        ...
        for d in range(num_branches):
            ...
            self.revert_projs.append(msnn.SequentialCell([
                tmp
            ]))
    ...
    def construct(self, x: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        proj_cls_token = torch.jit.annotate(List[torch.Tensor], [])  # 'torch.jit.annotate' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
