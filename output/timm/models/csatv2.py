...
def _dct_kernel_type_3(
    ...
    return mint.linalg.inv(_dct_kernel_type_2(kernel_size, orthonormal, device, dtype))  # 'torch.linalg.inv':默认参数名不一致(position 0): PyTorch=A, MindSpore=input;
...
class LearnableDct2d(msnn.Cell):
    ...
    def __init__(
        ...
        self.register_buffer('mean', ms.Tensor(_DCT_MEAN), persistent=False)  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;; 'torch.tensor':没有对应的mindspore参数 'device' (position 2);
        self.register_buffer('var', ms.Tensor(_DCT_VAR), persistent=False)  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;; 'torch.tensor':没有对应的mindspore参数 'device' (position 2);
        # Shape (3, 1, 1) for BCHW broadcasting
        self.register_buffer('imagenet_mean', ms.Tensor([0.485, 0.456, 0.406]).view(3, 1, 1), persistent=False)  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;; 'torch.tensor':没有对应的mindspore参数 'device' (position 2);
        self.register_buffer('imagenet_std', ms.Tensor([0.229, 0.224, 0.225]).view(3, 1, 1), persistent=False)  # 'torch.tensor':默认参数名不一致(position 0): PyTorch=data, MindSpore=input_data;; 'torch.tensor':没有对应的mindspore参数 'device' (position 2);
