...
class NaFlexEmbeds(msnn.Cell):
    ...
    def _apply_learned_naflex_pos_embed_grid_sample(
        ...
        theta = mint.zeros(B, 2, 3, device=device, dtype=ms.float32)
        theta[:, 0, 0] = scale_x
        theta[:, 1, 1] = scale_y
        ...
        bi = mint.arange(B, device=device, dtype=ms.int64).unsqueeze(1)
    ...
    def _apply_factorized_naflex_pos_embed_grid_sample(
        ...
        def _interp1d(table: ms.Tensor, scale: ms.Tensor, out_length: ms.Tensor) -> ms.Tensor:
            ...
            theta = mint.zeros(B, 2, 3, device=x.device)
            theta[:, 0, 0] = scale
            theta[:, 0, 2] = scale - 1
            theta[:, 1, 1] = 1
...
def create_attention_mask(
    ...
    if num_prefix_tokens > 0:
        ...
        prefix_valid = patch_valid.new_ones((B, num_prefix_tokens), dtype=ms.bool_)
    ...
    mask_float.masked_fill_(~mask_bool, ms.Tensor.min)  # 'torch.finfo' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
def global_pool_naflex(
    ...
    if pool_type == 'avg':
        ...
        masked_x = x.clone()
        masked_x[~patch_valid] = ms.Tensor.min  # 'torch.finfo' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class NaFlexVit(msnn.Cell):
    ...
    def _generate_rope_naflex(
        ...
        rope_embeds = mint.zeros(B, seq_len, self.rope.dim * 2, dtype=x.dtype, device=x.device)
