...
class Nest(msnn.Cell):
    ...
    def __init__(
        ...
        self.num_blocks = (4 ** mint.arange(num_levels, dtype = torch.long)).flip(0).tolist()  # 'torch.arange':没有对应的mindspore参数 'device' (position 6);
        ...
        for i in range(len(self.num_blocks)):
            ...
            prev_dim = dim
            curr_stride *= 2
        self.levels = msnn.SequentialCell([
            levels
        ])
