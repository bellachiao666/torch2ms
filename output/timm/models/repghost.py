...
class RepGhostModule(msnn.Cell):
    def __init__(
        ...
        self.fusion_conv = msnn.SequentialCell([
            fusion_conv
        ])
        self.fusion_bn = msnn.SequentialCell([
            fusion_bn
        ])
    ...
    def _fuse_bn_tensor(conv, bn, in_channels=None, device=None):
        ...
        if isinstance(conv, nn.Conv2d):
            ...
            kernel = mint.ones(size = (in_channels, 1, 1, 1))  # 'torch.ones':没有对应的mindspore参数 'device' (position 4);
