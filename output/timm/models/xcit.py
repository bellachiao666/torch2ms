...
class PositionalEncodingFourier(msnn.Cell):
    ...
    def construct(self, B: int, H: int, W: int):
        ...
        y_embed = mint.arange(1, H + 1).to(ms.float32).unsqueeze(1).repeat(1, 1, W)  # 'torch.arange':没有对应的mindspore参数 'device' (position 6);
        x_embed = mint.arange(1, W + 1).to(ms.float32).repeat(1, H, 1)  # 'torch.arange':没有对应的mindspore参数 'device' (position 6);
        y_embed = y_embed / (y_embed[:, -1:, :] + self.eps) * self.scale
        x_embed = x_embed / (x_embed[:, :, -1:] + self.eps) * self.scale
        dim_t = mint.arange(self.hidden_dim).to(ms.float32)  # 'torch.arange':没有对应的mindspore参数 'device' (position 6);
