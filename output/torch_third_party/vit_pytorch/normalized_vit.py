...
class NormLinear(msnn.Cell):
    def __init__(
        ...
        )  # 'torch.nn.utils.parametrize.register_parametrization' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class nViT(msnn.Cell):
    ...
    def __init__(
        ...
        *,
        ...
        self.residual_lerp_scales = nn.ParameterList([])  # 'torch.nn.ParameterList' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        ...
        for _ in range(depth):
            ...
            self.residual_lerp_scales.append(nn.ParameterList([
                ...
                ms.Parameter(mint.ones(dim) * residual_lerp_scale_init / self.scale),
            ]))  # 'torch.nn.ParameterList' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
