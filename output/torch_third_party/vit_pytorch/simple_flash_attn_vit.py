...
class Attend(msnn.Cell):
    def __init__(self, use_flash = False):
        ...
        device_properties = torch.cuda.get_device_properties(torch.device('cuda'))  # 'torch.device' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;; 'torch.cuda.get_device_properties' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
