...
class FiLM(msnn.Cell):
    def __init__(
        ...
        nn.init.zeros_(proj.weight)  # 'torch.nn.init.zeros_' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        nn.init.zeros_(proj.bias)  # 'torch.nn.init.zeros_' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class AST(msnn.Cell):
    ...
    def __init__(
        ...
        )  # 'torchaudio.transforms.Spectrogram' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class VAAT(msnn.Cell):
    def __init__(
        ...
        *,
        ...
        self.register_buffer('vit_layer_indices', tensor(vit_layer_indices), persistent = False)  # 'torch.tensor' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        ...
        self.register_buffer('ast_layer_indices', tensor(vit_layer_indices), persistent = False)  # 'torch.tensor' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
