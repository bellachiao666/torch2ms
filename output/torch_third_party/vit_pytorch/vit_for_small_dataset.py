...
class LSA(msnn.Cell):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.heads = heads
        self.temperature = ms.Parameter(mint.log(torch.tensor(dim_head ** -0.5)))  # 'torch.tensor' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
    ...
    def construct(self, x):
        ...
        dots = mint.matmul(q, k.transpose(-1, -2)) * self.temperature.exp()

        mask = mint.eye(dots.shape[-1], dtype = ms.bool)  # 'torch.eye':没有对应的mindspore参数 'device' (position 5);
        mask_value = -torch.finfo(dots.dtype).max  # 'torch.finfo' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
...
class SPT(msnn.Cell):
    ...
    def construct(self, x):
        ...
        shifted_x = list(map(lambda shift: nn.functional.pad(x, shift), shifts))
        x_with_shifts = mint.cat((x, *shifted_x), dim = 1)
