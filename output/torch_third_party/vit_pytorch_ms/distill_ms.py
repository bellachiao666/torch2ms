...
class DistillWrapper(msnn.Cell):
    ...
    def construct(self, img, labels, temperature = None, alpha = None, **kwargs):
        ...
        loss = F.cross_entropy(student_logits, labels)  # 'torch.nn.functional.cross_entropy' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;

        if not self.hard:
            distill_loss = F.kl_div(
                mint.special.log_softmax(distill_logits / T, dim = -1),
                nn.functional.softmax(teacher_logits / T, dim = -1).detach(),
            reduction = 'batchmean')  # 'torch.nn.functional.softmax.detach' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;; 'torch.nn.functional.kl_div' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
            distill_loss *= T ** 2

        else:
            teacher_labels = teacher_logits.argmax(dim = -1)
            distill_loss = F.cross_entropy(distill_logits, teacher_labels)  # 'torch.nn.functional.cross_entropy' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
