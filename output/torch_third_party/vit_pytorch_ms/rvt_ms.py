...
class AxialRotaryEmbedding(msnn.Cell):
    def __init__(self, dim, max_freq = 10):
        super().__init__()
        self.dim = dim
        scales = mint.linspace(1., max_freq / 2, self.dim // 4)
    ...
    def construct(self, x):
        ...
        seq = mint.linspace(-1., 1., steps = n)  # 'torch.linspace':没有对应的mindspore参数 'device' (position 6);
        seq = seq.unsqueeze(-1)
        ...
        sin = mint.cat((x_sinu.sin(), y_sinu.sin()), dim = -1)
        cos = mint.cat((x_sinu.cos(), y_sinu.cos()), dim = -1)
...
class SpatialConv(msnn.Cell):
    def __init__(self, dim_in, dim_out, kernel, bias = False):
        ...
        self.cls_proj = nn.Linear(dim_in, dim_out) if dim_in != dim_out else msnn.Identity()

    def construct(self, x, fmap_dims):
        cls_token, x = x[:, :1], x[:, 1:]
...
class GEGLU(msnn.Cell):
    def construct(self, x):
        x, gates = x.chunk(2, dim = -1)
        return nn.functional.gelu(gates) * x
...
class Attention(msnn.Cell):
    ...
    def construct(self, x, pos_emb, fmap_dims):
        ...
        if self.use_rotary:
            ...
            q, k = map(lambda t: mint.cat(t, dim = -1), ((q, q_pass), (k, k_pass)))

            # concat back the CLS tokens

            q = mint.cat((q_cls, q), dim = 1)
            k = mint.cat((k_cls, k), dim = 1)
