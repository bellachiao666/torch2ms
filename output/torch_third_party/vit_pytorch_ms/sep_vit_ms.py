...
class DSSA(msnn.Cell):
    def __init__(
        ...
        self.to_qkv = nn.Conv1d(dim, inner_dim * 3, 1, bias = False)  # 'torch.nn.Conv1d' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        ...
        ])  # 'torch.nn.Conv1d' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
