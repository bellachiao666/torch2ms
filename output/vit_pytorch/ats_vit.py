...
class AdaptiveTokenSampling(msnn.Cell):
    ...
    def construct(self, attn, value, mask):
        ...
        mask_value = -torch.finfo(attn.dtype).max / 2  # 'torch.finfo' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
        ...
        unique_sampled_token_ids = pad_sequence(unique_sampled_token_ids_list, batch_first = True)  # 'torch.nn.utils.rnn.pad_sequence' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;
