...
class Tokenizer(msnn.Cell):
    def __init__(self,
        ...
        ])  # 'torch.nn.MaxPool2d' 未在映射表(api_mapping_out_excel.json)中找到，需手动确认;

        self.apply(self.init_weight)

    def sequence_length(self, n_channels=3, height=224, width=224):
        return self.forward(mint.zeros((1, n_channels, height, width))).shape[1]
...
class TransformerClassifier(msnn.Cell):
    def __init__(self,
        ...
        if not seq_pool:
            sequence_length += 1
            self.class_emb = ms.Parameter(mint.zeros(size = (1, 1, self.embedding_dim)), requires_grad=True)
        else:
        ...
        if positional_embedding == 'none':
            ...
            self.positional_emb = ms.Parameter(sinusoidal_embedding(sequence_length, embedding_dim),
                                               requires_grad=False)
