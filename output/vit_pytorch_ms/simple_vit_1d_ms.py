...
def posemb_sincos_1d(patches, temperature = 10000, dtype = torch.float32):
    ...
    n = ops.arange(start = n)  # 'torch.arange':没有对应的mindspore参数 'out';; 'torch.arange':没有对应的mindspore参数 'layout';; 'torch.arange':没有对应的mindspore参数 'device';; 'torch.arange':没有对应的mindspore参数 'requires_grad';
    assert (dim % 2) == 0, 'feature dimension must be multiple of 2 for sincos emb'
    omega = ops.arange(start = dim // 2) / (dim // 2 - 1)  # 'torch.arange':没有对应的mindspore参数 'out';; 'torch.arange':没有对应的mindspore参数 'layout';; 'torch.arange':没有对应的mindspore参数 'device';; 'torch.arange':没有对应的mindspore参数 'requires_grad';
    omega = 1. / (temperature ** omega)

    n = n.flatten()[:, None] * omega[None, :]
    pe = ops.cat(tensors = (n.sin(), n.cos()), dim = 1)  # 'torch.cat':没有对应的mindspore参数 'out';
